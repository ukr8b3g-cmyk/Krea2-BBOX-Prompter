...
Some effects may be weak or may not appear at all depending on the subject, background complexity, prompt conflicts, model interpretation, and the available visual space in the scene. Treat Prompt Effect as a fast direction tool, not a guaranteed fixed filter. Recent additions include practical Light presets for sunlight and light direction, plus Weather presets for outdoor photographic scenes.
...
```text
https://github.com/ukr8b3g-cmyk/Krea2-BBOX-Prompter/blob/main/workflow/Krea2-BBOX-Node-Test.json
```

Other workflow files, if present, are in:
...
6. Add scene/background presets with 🖼️ Krea2 BBOX Background Effect when needed.
7. Connect the final prompt to CLIP Text Encode.
```
...
flowchart LR
    ...
    D -->|prompt_out| F["🖼️ Krea2 BBOX Background Effect<br/>Optional background prompt append"]
    F -->|prompt_out| E["CLIP Text Encode<br/>text"]
...
Angle: Auto / Front view / POV / Top POV / Side view / Low angle / High angle / Top-down view / Three-quarter view / Dutch angle / Over-the-shoulder / Rear view
```
...
```text
ComfyUI/custom_nodes/Krea2-BBOX-Prompter-Suite/user_presets/prompt_effect_custom_presets.json
```

If you move to another PC, another ComfyUI install, or another storage location, copy this JSON file to the same path in the new install. The file is created automatically the first time you save a Custom effect preset.
...
作者目線では、このスイートの中でもPrompt Effectノードが**一番楽しい部分**です。Krea2は短いスタイル指定やエフェクト指定にも意外とよく追従するため、これらのプリセットは**実際にかなり使えます**。カメラ風、ライティング、天候、カラーテーマ、フィルム風、構図補助などを少し加えるだけでも、最終画像の印象がかなり変わります。
...
現在のテンプレートワークフローはこちらです。

```text
https://github.com/ukr8b3g-cmyk/Krea2-BBOX-Prompter/blob/main/workflow/Krea2-BBOX-Node-Test.json
```

その他のワークフローファイルがある場合は、以下にあります。
...
6. 必要に応じて 🖼️ Krea2 BBOX Background Effect で背景プリセットを追加する
7. CLIP Text Encode に接続して生成する
```
...
## 📐 Krea2 BBOX Canvas
<img width="687" height="829" alt="Clip_15" src="https://github.com/user-attachments/assets/4bc41629-f5ef-473a-839c-ac3e748ea571" />
...
## 📝 Krea2 BBOX Prompter
<img width="422" height="783" alt="Clip_16" src="https://github.com/user-attachments/assets/1a8a5603-d50e-4748-8646-cf88aa012f76" />
...
別PC、別ComfyUI、別ストレージへ移す場合は、このJSONファイルを同じ場所へコピーしてください。ファイルがない場合は、最初にCustom effect presetを保存した時に自動作成されます。
...
6. Background Effectは任意で、背景プリセットを重ねるときだけ使う
7. 生成結果を確認しながら、必要に応じてBBOXのサイズや位置、descを調整して画像生成を繰り返す
