...
The UI first derives the thumbnail path from the loaded extension JavaScript URL. If that cannot be detected, it falls back to both common folder-name variants so thumbnails can load from installs named either `Krea2-BBOX-Prompter` or `Krea2-BBOX-Prompter-Suite`.
